...
|----|--------|------|--------------|
| UPD-20260919-001 | pending | Create compact strand record buffer in `cosmic_web.rs` (replace `braid_segments` with `strand_records` returning `Vec<StrandRecord>` with endpoints, wander, strand params, rgba) | §In-scope P1 |
| UPD-20260919-002 | pending | Write `RIBBON_VERT` GLSL: expand `gl_VertexIndex` → strand/segment/corner; evaluate `braid_point(t0/t1)` in shader; camera-facing offset with world-space width + min-pixel clamp | §In-scope P1 |
| UPD-20260919-003 | pending | Write `RIBBON_FRAG` GLSL: lateral Gaussian falloff, density-graded emissive, premultiplied additive output | §In-scope P1 |
| UPD-20260919-004 | pending | Build `ribbon_pipeline` in `main.rs`: `TriangleList`, additive One/One, no depth write, scene + LDR variants; push constant with MVP, px_scale, viewport size, redshift | §In-scope P1 |
| UPD-20260919-005 | pending | Update `upload_cosmic_braid` to upload strand records instead of baked vertices; update draw call to TriangleList with strand_count × 60 verts | §In-scope P1 |
| UPD-20260919-006 | pending | Delete/refactor old `webline` pipeline + `WEBLINE_VERT/FRAG` shaders; update both surfaces (Game Demo + Cosmic Web tab) to use ribbon pipeline | §In-scope P1 |
...
| UPD-20260919-009 | pending | Dim low-density braid links (alpha floor, rgb floor) so voids read dark; retune palette in `cosmic_web.rs` ramps | §In-scope P4 |
| UPD-20260919-010 | pending | Tune `BloomParams` (threshold/intensity) + resolve push constants (exposure) + `COSMIC_REDSHIFT_PER_MPC` so gold survives at depth | §In-scope P4 |
| UPD-20260919-011 | pending | Screenshot both surfaces (before + after); verify DoD criteria; record frame-time on UHD 620 @1080p | §DoD |
| UPD-20260919-012 | pending | Update docs: `docs/techstack/rendering.md` (ribbon pipeline), architecture report, `cosmic-enhancement.md` roadmap (items landed), `docs/techstack/README.md` version line; `quality.md` if 3rd blur scale added | §DoD |
| UPD-20260919-013 | pending | Run quality gates: `cargo fmt --check`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo build --workspace`, `cargo test --workspace --all-targets`, `cargo test --doc --workspace`, headless viewer checks | §DoD |

## DoD verification

| DoD # | Criterion (from notion.md) | Status | Evidence |
|-------|----------------------------|--------|----------|
| 1 | Filaments are camera-facing ribbons with soft lateral falloff; no 1px wireframe visible — both surfaces | ⬜ | UPD-20260919-011 screenshot |
| 2 | Dense filaments bloom; voids read dark — before/after | ⬜ | UPD-20260919-011 screenshot |
| 3 | Node cores gold, halos virial-radius, no 256px clamp up close | ⬜ | UPD-20260919-011 screenshot |
| 4 | Descriptor hash unchanged; determinism tests green | ⬜ | UPD-20260919-013 test output |
| 5 | All quality.md gates green | ⬜ | UPD-20260919-013 output |
| 6 | No frame-time regression UHD 620 @1080p; bloom write-once rule | ⬜ | UPD-20260919-011 perf |
| 7 | Docs synced, links resolve | ⬜ | UPD-20260919-012 |
