...
- [x] ADR-025: field-based cosmic-web render — non-hashed `WebField` sidecar (Zel'dovich tracers + density/class grid) exported from `engine::universe::web`, rendered as adaptive tracer splats + depth window + hub hierarchy + grid gas veil (sprites Low / raymarch Med+High) + write-once mip bloom + vista intro; link graph stays the gameplay authority; no hash/save change (extends ADR-023, lifts ADR-024's engine-untouched rule for one export) — [`ADR-025.md`](ADR-025.md), lands with `plans/v0.3.3/` (eight features).
- [x] ADR-026: sphere-bounded descriptor (`UNIVERSE_VERSION` 3→4), procedural GPU tracers from an exported displacement grid (`WebField.tracers` retires), non-blocking origin rebase on a worker thread, transfer-function grading + compact hubs + interior vista framing (extends ADR-025/023/013) — [`ADR-026.md`](ADR-026.md), lands with `plans/v0.3.4/` (six features).
- [x] ADR-027: cosmic performance — measure first (frame timing), then tier by device; captures pin `--tier` (default High) — [`ADR-027.md`](ADR-027.md), lands with `plans/v0.3.5/` (two features).
