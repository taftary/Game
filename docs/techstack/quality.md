...
| Cosmic bloom per frame, cosmic views only, debug shell (update-2026-09-18-2328; mip pyramid since v0.3.3 `bloom-mip-chain`; tier-selected since v0.3.5 `cosmic-device-tier`) | prefilter + downs + pass-through + tent ups over the tier's levels (`scene + 2·levels` HDR images); `BLOOM=0` resolves scene + `down[0]`; LDR bypass draws the same layouts direct (no post) | same (pyramid runs the boot/cycled tier in the debug binary: 3 Low / 4 Medium / 5 High) | same |
| Cosmic-web cue raymarch (v0.2.0) | 64x64 source grid, ≤16 steps, analytic fallback available | 128x128, ≤32 steps | 256x256, ≤64 steps |
| Cosmic field render (v0.3.4 `cosmic-gpu-tracers`, ADR-026 §2 — procedural, shipped 2026-09-23; tier-selected in the debug shell since v0.3.5 `cosmic-device-tier`: `Cpu`/`VirtualGpu` → Low, `IntegratedGpu` → Medium, `DiscreteGpu` → High, `GAME_DEBUG_TIER` pins, `F4` cycles, `--tier` pins captures) | no tracer vertex buffers (0 B vs 16 MB v0.3.3 High); draws are `cells × k` index-driven invocations — Low 1 072 958 (k=1), Medium 2 145 916 (k=2), High 8 583 664 (k=8); hubs 5574 impostors + 17 070 members; veil = cell sprites ≈ 168k drawn (stride-2 of 335 387, ≈ 6.0 MB, no raymarch — shipped); bloom 5 levels, `scene + 2·5` images ≈ 14 MB at 1080p High (shipped); `WebField` sidecar 16.8 MB displacement + 2 MB grid + ≈ 4.4 MB cell list (18 MB, NFR2 ≤ 24 MB); `cells_ms` 46.6 ms release nominal (NFR3 ≤ 50 ms); vertex-stage texture fetch is a required device feature (Vulkan 1.0 core; mobile guards check it); UHD 620 inspector (seed 1337, 1408×768, dev): High prepass ≈ 120–140 ms → Medium ≈ 31–33 ms (3–4×, `cosmic_timing=`) | splats ≤ 1.0M; veil = quarter-res raymarch 32 steps ≈ 4.1M fetches/frame at 1080p (one 2 MB R8 3D image + ≈ 1 MB quarter-res HDR target — shipped); bloom 4 levels | proc splats all cells ×8 (+ optional `refine(2)` retired); raymarch 48 steps ≈ 6.2M fetches/frame at 1080p (shipped); bloom 5 levels |
...
| Cosmic void contrast (v0.3.4 `cosmic-void-contrast`, ADR-026 §4 — transfer-grading, done 2026-09-23) | shared `cosmic_transfer` (floor 0 / band 2.5 / knots slope 1 / rim 30 Mpc) across proc splats + march + sprite colours (CPU mirror; alpha untouched); MAP splat gain 0.25 (graded); HDR clear deep indigo `(0.02, 0.02, 0.06)`, resolve adds no backdrop; UHD 620 slab scans (seed 1337): void floor 1.00× backdrop, knot ridge 8.0×, wall sheet 1.55×; zero new fetches, ≤ 6 ALU per vertex/step; fragment arithmetic-only, write-once untouched | same | same |
| Cosmic hub compacts (v0.3.4 `cosmic-hub-compact-cores`, ADR-026 §5 — px-capped cores, done 2026-09-23) | pin ≤ 4 px + Tier A core ≤ 10 px + Tier B core ≤ 6 px in `GLOW_VERT` (CPU mirror; retired world halos, suffusion `1.0·r_vir` α 0.04 only where ≥ 12 px); members `150+250·l` (A) / `30+40·l` (B), emissive `2+2·l`, α 0.95, 1.5–2.5 px, ≤ 100k (nominal ≈ 46.5k; headless 5030 impostors + 38 795 members); glow ≤ 120k pts (≈ 3.8 MB; march-mode 43 825, sprites-mode ≈ 212k with veil stride-2 — Low draws stride-2); rebase worker build 9.3 ms dev-profile (glow only, off-frame, ≤ 40 ms); UHD 620 slab (seed 1337): core 8 px, blazing A 5/49 CPU in-slice, halo 1.17×, 95 dots + 5/20 Tier A crops; demo spawn core 7 px + 262 dots, approach core 10 px (caps hold close), no wash | same | same |
