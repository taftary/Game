...
| Cosmic rebase (v0.3.4 `cosmic-rebase-async`, ADR-026 §3; glow-only since `cosmic-gpu-tracers` CGT-010) | no frame > 33 ms attributable to rebase; main-thread cost per rebase = upload + swap of the demo glow buffer only (≈ 1 MB; the 17 MB splat upload retired — origin rides a push constant); headless traverse (507 Mpc, 10 rebases): max tick 0.68 ms dev-profile (frame-work analog, ≤ 33 ms gate); worker compute 5.5 ms dev-profile (glow only; was ≈ 13.8 s with splats), release 384–487 ms (off-frame) | same | same |
