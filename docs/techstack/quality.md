...
| Cosmic field render (v0.3.3, ADR-025 — veil shipped 2026-09-22) | tracer splats ≤ 300k pts (≤ 5 MB/surface — shipped: stride gives 255 900, 4.1 MB, overdraw 2.0×; no new draw/pass); hubs 6720 impostors + ≤ 40k members (shipped, net +9k pts vs v0.3.2); veil = cell sprites ≈ 168k drawn (stride-2 of 335 387, ≈ 6.0 MB, no raymarch — shipped); bloom 5 levels, `scene + 2·5` images ≈ 14 MB at 1080p High (shipped); `WebField` sidecar ≤ 20 MB, boot +≤ 120 ms (shipped: 17 MB, +0 ms) | splats ≤ 1.0M; veil = quarter-res raymarch 32 steps ≈ 4.1M fetches/frame at 1080p (one 2 MB R8 3D image + ≈ 1 MB quarter-res HDR target — shipped); bloom 4 levels | splats all tracers (+ optional `refine(2)`); raymarch 48 steps ≈ 6.2M fetches/frame at 1080p (shipped); bloom 5 levels |
| Cosmic rebase (v0.3.4 `cosmic-rebase-async`, ADR-026 §3) | no frame > 33 ms attributable to rebase; main-thread cost per rebase = upload + swap of the two demo buffers only (≈ 17 MB splats + ≈ 1 MB glow; split over two frames if > 8 ms — measured on reference HW in CRA-008); headless traverse (507 Mpc, 10 rebases): max tick 1.90 ms dev-profile (frame-work analog, ≤ 33 ms gate); worker compute ≈ 13.8 s dev-profile (off-frame, release far faster) | same | same |
